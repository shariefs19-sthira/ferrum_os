...
export default function SutraPanel({ onSubmit }: { onSubmit:(text:string)=>void }) {
  ...
  return <aside className="flex min-h-[22rem] flex-col border border-relume-border bg-relume-command text-white lg:h-full" aria-label="SUTRA design assistant" data-sutra-panel data-guided-stage={stage} data-demo-paused={demoPaused}>
    <header className="border-b border-white/15 px-4 py-3"><div className="flex items-center justify-between gap-3"><h1 className="font-heading text-lg font-semibold">SUTRA</h1><span className="rounded-full border border-relume-accent px-2 py-1 text-[10px] font-bold tracking-wider text-relume-accent">INDICATIVE</span></div><p className="mt-1 text-xs text-white/65">Constrained choices over the deterministic workspace.</p></header>
