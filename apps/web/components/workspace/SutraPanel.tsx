...
import { commandEvents, type SutraEvent, type SutraInputSource } from "../../lib/sutra/events"
...
export default function SutraPanel({ onEvent, onClose }: { onEvent:(event:SutraEvent)=>void; onClose?:()=>void }) {
  ...
  const run = useCallback((raw:string,source:SutraInputSource="text",isDemo=false) => { const command=raw.trim(); if(!command)return; if(!isDemo)commandEvents(command,source).forEach(event=>onEvent(event)); setMessages(current=>[...current.slice(-5),{id:nextId.current++,role:"operator",text:isDemo?`Demo preview: ${command}`:command},{id:nextId.current++,role:"sutra",...answer(command)}]); setValue("") },[onEvent])
  const choose = (label:string,command:string) => { const currentStage=stage; run(command,"chip"); onEvent({type:"STATE_DELTA",path:"guided.selection",value:{stage:currentStage,label}}); setSelected(current=>({...current,[currentStage]:label})); const index=stages.indexOf(currentStage); if(index<stages.length-1){setHistory(current=>[...current,currentStage]);setStage(stages[index+1])} }
  ...
  return <aside className="flex h-full min-h-[22rem] flex-col border border-relume-border bg-relume-command text-white" aria-label="SUTRA design assistant" data-sutra-panel data-guided-stage={stage} data-demo-paused={demoPaused} data-guided-open={guidedOpen}>
    <header className="flex items-start justify-between gap-3 border-b border-white/15 px-4 py-3">
      <div>
        <div className="flex items-center gap-3"><h1 className="font-heading text-lg font-semibold">SUTRA — your digital Sthapati</h1><span className="rounded-full border border-relume-accent px-2 py-1 text-[10px] font-bold tracking-wider text-relume-accent">INDICATIVE</span></div>
        <p className="mt-1 text-xs text-white/65">Constrained choices over the deterministic workspace.</p>
      </div>
      {onClose && <button type="button" onClick={onClose} aria-label="Close SUTRA" className="min-h-11 shrink-0 rounded-full border border-white/25 px-3 text-xs font-semibold hover:border-relume-accent focus-visible:outline focus-visible:outline-2 focus-visible:outline-relume-accent">Close</button>}
    </header>
