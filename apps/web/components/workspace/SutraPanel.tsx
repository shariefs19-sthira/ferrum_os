...
export default function SutraPanel({ onSubmit }: { onSubmit:(text:string)=>void }) {
  ...
  const run = useCallback((raw:string,isDemo=false) => { const command=raw.trim(); if(!command)return; if(!isDemo)onSubmit(command); setMessages(current=>[...current.slice(-5),{id:nextId.current++,role:"operator",text:isDemo?`Demo preview: ${command}`:command},{id:nextId.current++,role:"sutra",...answer(command)}]); setValue("") },[onSubmit])
