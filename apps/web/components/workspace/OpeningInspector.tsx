...
export default function OpeningInspector({ opening, onCommit, doorCount, windowCount }: { opening?: StudioOpening; onCommit: (edit: OpeningEdit) => string | undefined; doorCount: number; windowCount: number }) {
  const [draft, setDraft] = useState<Draft | undefined>(opening ? draftFrom(opening) : undefined)
  const [message, setMessage] = useState('')
  useEffect(() => { setDraft(opening ? draftFrom(opening) : undefined); setMessage('') }, [opening])
  ...
  return <aside className="absolute bottom-4 right-4 z-30 max-h-[calc(100%-7rem)] w-[min(22rem,calc(100%-2rem))] overflow-y-auto rounded-relume border border-relume-border bg-white p-4 shadow-sm" aria-label={`${label} property inspector`} data-opening-inspector>
    <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-relume-muted">Selected opening</p>
    ...
    <div className="mt-4 grid grid-cols-2 gap-3">
      {(['widthM', 'heightM'] as const).map((field) => <label key={field} className="text-xs font-medium text-relume-ink">{field === 'widthM' ? 'Width' : 'Height'} (m)<input type="number" min="0" step="0.05" value={draft[field]} onChange={(event) => setDraft({ ...draft, [field]: event.target.value })} onBlur={() => commit(field)} className="mt-1 min-h-11 w-full rounded-relume border border-relume-border px-3 text-sm text-relume-ink" aria-describedby={message ? 'opening-inspector-message' : undefined} /></label>)}
      {opening.kind === 'window' && <label className="text-xs font-medium text-relume-ink">Sill (m)<input type="number" min="0" step="0.05" value={draft.sillM} onChange={(event) => setDraft({ ...draft, sillM: event.target.value })} onBlur={() => commit('sillM')} className="mt-1 min-h-11 w-full rounded-relume border border-relume-border px-3 text-sm text-relume-ink" aria-describedby={message ? 'opening-inspector-message' : undefined} /></label>}
      <label className="col-span-2 text-xs font-medium text-relume-ink">Configuration<select value={draft.configuration} onChange={(event) => { const next = { ...draft, configuration: event.target.value }; setDraft(next); const result = onCommit({ configuration: event.target.value as StudioOpening['configuration'] }); setMessage(result ?? '') }} className="mt-1 min-h-11 w-full rounded-relume border border-relume-border bg-white px-3 text-sm text-relume-ink">{(opening.kind === 'door' ? [['single-swing', 'Single swing'], ['double-swing', 'Double swing'], ['sliding', 'Sliding']] : [['fixed', 'Fixed'], ['casement', 'Casement'], ['sliding', 'Sliding']]).map(([value, text]) => <option key={value} value={value}>{text}</option>)}</select></label>
    </div>
