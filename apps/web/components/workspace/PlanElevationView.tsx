...
function OpeningMarker({ opening, room, selected, onSelect }: { opening: StudioOpening; room: StudioRoom; selected: boolean; onSelect?: () => void }) {
  ...
  const hit = { x: rect.x - 0.22, y: rect.y - 0.22, width: rect.width + 0.44, height: rect.height + 0.44 }
  const select = () => onSelect?.()
  const keySelect = (event: KeyboardEvent<SVGRectElement>) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); select() } }
  return <g><rect {...rect} aria-hidden="true" fill={opening.kind === 'door' ? '#FFFFFF' : '#B8D0DD'} stroke={selected ? '#161616' : '#52616B'} strokeWidth={selected ? 0.2 : 0.1} pointerEvents="none" /><rect {...hit} role="button" tabIndex={0} data-opening-id={opening.id} data-opening-kind={opening.kind} aria-label={`Select ${opening.kind}, ${opening.widthM.toFixed(2)} metre wide`} aria-pressed={selected} fill="transparent" stroke="transparent" className="cursor-pointer focus:outline-none" onClick={select} onKeyDown={keySelect} /></g>
}
...
export default function PlanElevationView({ plan, view, activeFloor, selectedOpeningId, onSelectOpening }: PlanElevationViewProps) {
  ...
  const visibleEdges = elevationId === 'north' ? ['north', 'south'] : ['east', 'west']
  const elevationOpenings = openings.filter((opening) => opening.floor === activeFloor && visibleEdges.includes(opening.hostEdge))
  return <svg viewBox={`${-padding} -1 ${elevation.widthM + padding * 2} ${elevation.heightM + 2}`} className="h-full min-h-[24rem] w-full" role="img" aria-label={`${elevation.name}, ${plan.floors} floors`}><rect x="0" y="0" width={elevation.widthM} height={elevation.heightM} fill="#DCE8EF" stroke="#0B1F3A" strokeWidth="0.12" />{elevation.floorLinesM.map((height) => <line key={height} x1="0" x2={elevation.widthM} y1={height} y2={height} stroke="#52616B" strokeWidth="0.08" />)}{elevationOpenings.map((opening) => { const room = plan.rooms.find((candidate) => candidate.id === opening.roomId); if (!room) return null; const x = elevationId === 'north' ? room.xM + opening.positionM : room.yM + opening.positionM; const y = (opening.floor - 1) * plan.floorHeightM + plan.floorHeightM - opening.sillM - opening.heightM; const selected = opening.id === selectedOpeningId; return <rect key={opening.id} x={x} y={y} width={opening.widthM} height={opening.heightM} role="button" tabIndex={0} data-opening-id={opening.id} data-opening-kind={opening.kind} aria-label={`Select ${opening.kind} in ${elevation.name}`} aria-pressed={selected} fill={opening.kind === 'door' ? '#FFFFFF' : '#B8D0DD'} stroke={selected ? '#161616' : '#0B1F3A'} strokeWidth={selected ? 0.12 : 0.06} className="cursor-pointer focus:outline-none" onClick={() => onSelectOpening?.(opening.id)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); onSelectOpening?.(opening.id) } }} /> })}<line x1={-padding} x2={elevation.widthM + padding} y1={elevation.heightM} y2={elevation.heightM} stroke="#138808" strokeWidth="0.12" /></svg>
}
