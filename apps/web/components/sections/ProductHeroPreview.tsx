...
// Exact INDICATIVE rows shipped in migrations/0002_seed.sql.
const parcel = {
...
const rates = [
  { item: 'Cement (OPC 53)', unit: '50 kg bag', rate: 420 },
  { item: 'TMT Steel (Fe 500D)', unit: 'kg', rate: 68.5 },
  { item: 'Skilled Mason', unit: 'day', rate: 900 },
]
...
// Exact illustrative Karnataka row shipped in migrations/0003_transact.sql.
const transact = { consideration: 5_000_000, stampRate: 0.05, registrationRate: 0.01 }

function StatusBadge({ children = 'Indicative' }: { children?: ReactNode }) {
  return (
...
function PreviewShell({
  ...
  return (
    <figure
      data-product-preview={tool}
      aria-label={`${tool} ${status.toLowerCase()} preview`}
...
function RoadmapRows({ rows }: { rows: string[] }) {
  return (
    <ul className="space-y-2">
      {rows.map((row) => (
        <li key={row} className="flex items-center justify-between gap-4 border-b border-relume-border py-2 text-xs last:border-0">
          <span className="text-relume-ink">{row}</span>
          <span className="font-medium uppercase tracking-[0.12em] text-relume-muted">Not built</span>
        </li>
      ))}
    </ul>
  )
...
export default function ProductHeroPreview({ product }: { product: ProductPreview }) {
  if (product === 'landintel') {
    return (
      <PreviewShell tool="ULPIN parcel lookup" note="Seeded sample parcel — not live registry, title, or geometry data.">
        <p className="font-mono text-sm font-semibold text-relume-ink">{parcel.ulpin}</p>
        <dl className="mt-3">
          <DataRow label="District" value={parcel.district} />
          <DataRow label="Area" value={`${parcel.areaSqm.toLocaleString('en-IN')} m²`} />
  ...
  if (product === 'designstudio') {
    ...
    const floorPlate = (plotWidth - setback * 2) * (plotLength - setback * 2)
    return (
      <PreviewShell tool="Test-fit massing" note="Sample inputs: 20 × 30 m plot, two floors, 2 m setback — verify local controls.">
        <div className="grid grid-cols-[7rem_1fr] items-center gap-4">
          ...
          <dl>
            <DataRow label="Plot" value={`${plotArea} m²`} />
            <DataRow label="Floor area" value={`${floorPlate * floors} m²`} />
            <DataRow label="Coverage" value={`${((floorPlate / plotArea) * 100).toFixed(1)}%`} />
          </dl>
        </div>
      </PreviewShell>
    )
  ...
  if (product === 'boq-pro') {
    ...
    return (
      <PreviewShell tool="Ferrum rate band" note="Computed from the three seeded cement-rate samples — not a current market quote.">
        ...
        <p className="mt-2 text-3xl font-semibold tracking-relume-tight text-relume-ink">{inr(ferrumRate.band.p50)}</p>
  ...
  if (product === 'promarket') {
    return (
      <PreviewShell tool="Rate comparison" note="Seeded Bengaluru sample rates — not live supplier, labour, or market quotations.">
  ...
  if (product === 'procurehub') {
    return (
      <PreviewShell
        tool="Procurement workflow status"
        status="Roadmap"
        note="No live catalogue, supplier quote, purchase order, or transaction is presented here."
      >
        <RoadmapRows rows={['Supplier catalogue', 'RFQ comparison', 'Purchase orders']} />
      </PreviewShell>
    )
  }
  ...
  const inr = (value: number) => `₹${value.toLocaleString('en-IN', { maximumFractionDigits: 0 })}`
  return (
    <PreviewShell tool="Stamp-duty estimate" note="Illustrative Karnataka seed rate — verify every figure with the local sub-registrar.">
      <p className="text-sm font-semibold text-relume-ink">Karnataka · sample consideration</p>
      <dl className="mt-3">
        <DataRow label="Consideration" value={inr(transact.consideration)} />
        <DataRow label="Stamp duty (5%)" value={inr(transact.consideration * transact.stampRate)} />
        <DataRow label="Registration (1%)" value={inr(transact.consideration * transact.registrationRate)} />
      </dl>
    </PreviewShell>
  )
}
