...
export const productFeatureRegistry: Record<CockpitProduct, ProductFeature[]> = {
  ...
  designstudio: [
    available('regional-shell-catalog', 'Regional building-shell catalogue', 'Swap original, typology-derived outer shells directly in the rendered cockpit. The starter catalogue covers Indian regional and climate responses, records provenance and copyright boundaries, and labels every model INDICATIVE. It does not reproduce an architect project or establish planning approval, engineering validity or constructability.', ['building shell', 'regional architecture', 'model swap', 'typology', 'kerala courtyard', 'mangalore tiles', 'goa architecture'] ),
    available('parcel-shell-recommendation', 'Parcel-aware SUTRA shell recommendation', 'When a LandIntel parcel is locked, DesignStudio ranks shell studies from recorded state, plot area and land use, explains the match and preserves UNKNOWN planning, orientation, access, terrain and adjoining conditions. With no locked parcel it fails to a neutral comparative shell rather than inventing locality data.', ['sutra recommendation', 'parcel context', 'locality', 'building type', 'land handoff']),
    available('open-rendering-stack', 'Open rendering stack', 'Three.js provides the live browser PBR viewport, Three GPU Path Tracer provides an optional progressive beauty preview, and web-ifc preserves IFC geometry and properties. A controlled Blender Cycles worker is the next high-resolution render stage. D5 and V-Ray are optional user-installed export plugins only and receive no Ferrum project write access.', ['three.js', 'path tracing', 'cycles', 'web-ifc', 'open source rendering', 'beauty render']),
    ...
    available('model-preview-on-ingestion', 'Model preview on ingestion', 'The available first slice selects an IFC revision locally in the browser, computes its SHA-256, and uses web-ifc to report schema, declared length unit, coordination origin and parsed entity counts. The file is not uploaded or retained. A future successful render will remain PREVIEWED only; geometry viewing, bounds, CRS, vertical datum, revision comparison, validation and machine release remain blocked and are stated as unresolved in the report.', ['model preview', 'ingestion', 'ifc', 'openbim', 'web-ifc', 'checksum']),
    ...modelInspectionCapabilities.map((capability) => capability.id === 'ifc-metadata'
      ? available(`model-inspection-${capability.id}`, capability.title, capability.body, capability.keywords)
      : roadmap(`model-inspection-${capability.id}`, capability.title, `${capability.body} This capability remains roadmap work and is not connected to the current metadata-only IFC intake.`, capability.keywords)),
  ],
  structura: [
