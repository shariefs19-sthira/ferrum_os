...
## WAVE PROTOCOL
Human labels 10-20 tasks as {id, J-type, file scope, tier, priority}. Disjoint-scope check mandatory before launch. Tasks sit in a LABELED QUEUE; agents (existing or newly onboarded) PULL the highest-priority task matching their declared tier; human may override. Build parallel; land serial (merge queue).
