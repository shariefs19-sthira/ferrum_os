...
| Paperhouses | https://www.paperhouses.com/ (fetch failed, curl 000; no primary official page identified) | UNKNOWN (presumed printable paper models) | UNKNOWN. Third-party paper-craft sites in search often say personal use only; that is not evidence for this source. | UNKNOWN | N | N | U (do not use until identified) |
...
5. Google 3D Tiles: live viewer only, never stored in the catalog. Others (Great Buildings, UW CBD, NDSU, Paperhouses): link/reference only, or seek written permission.
...
## Limitations
Summaries came from an LLM-assisted fetch, not manual reading; Great Buildings model terms and Paperhouses are unverified; NDSU/MWPS rights not found. Re-verify before any ingestion.
