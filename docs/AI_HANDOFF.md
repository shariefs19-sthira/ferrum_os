...
**Current State:**
*   The `main` branch contains all the updated documentation for the agent system, standards, task structure, dispatch, and prophecy.
*   The `docs/AGENT_REGISTRY.md` lists all known agents, including the new Dispatcher and Prophet.
...
**Next Steps:**
*   The designated [POS:DISPATCHER] agent should continue to annotate tasks in `WAVE_QUEUE.md` with recommended models, considering prophecies from `PROPHECY_LOG.md`.
*   The [POS:PROPHET] agent should make its first active prophecy at the next wave boundary or after a significant event.
*   Agents should continue to follow the PREPARE -> EXECUTE -> LOG structure.
