...
## 2026-09-03 10:30 - SCRIBE RULE 21 (Self-verifying tools + living resume) added + four RESUME templates created + seats mirror
**Action:** Added AGENTS.md RULE 21, three parts: (1) batch tools self-verify — land.ps1 and any sweep/audit/migration script emits processed/landed/skipped/held counts and a nonzero exit or explicit HELD state whenever work remains; zero-processed "success" against a non-empty queue is a FAILURE, never a pass; (2) disk-verify before reliance — a "reviewed"/"trusted"/"landed" claim is checked against git log/diff at the moment of reliance, generalizing the W2-357 never-merged scar from code review to tooling and process claims broadly; (3) living resume — every seat maintains docs/RESUME_<SEAT>.md, updated every turn (done SHAs, in-flight, next, blockers); after a limit event or API error the new session reads its own resume file FIRST, and conductor resume prompts are generated from that file's content, never reconstructed from chat memory. Created the four resume templates this branch chain's roster actually has seat docs for: docs/RESUME_CRANE.md, RESUME_ATLAS.md, RESUME_CODEX.md, RESUME_SCRIBE.md — all empty except RESUME_SCRIBE.md, which SCRIBE populated with its own real done/in-flight/next/blockers state this turn (practicing the rule immediately rather than just writing it). Noted on RESUME_CODEX.md that the seat is mid-rename to MASON in a separate branch and the resume file should be renamed alongside that reconciliation. Mirrored RULE 21 into docs/seats/CRANE.md, ATLAS.md, CODEX.md, SCRIBE.md.
**By:** SCRIBE (Claude Code)
**Status:** ✅ Complete
**Files Modified:** AGENTS.md, docs/RESUME_CRANE.md (new), docs/RESUME_ATLAS.md (new), docs/RESUME_CODEX.md (new), docs/RESUME_SCRIBE.md (new), docs/seats/ATLAS.md, docs/seats/CODEX.md, docs/seats/CRANE.md, docs/seats/SCRIBE.md, docs/ACTIVITY_LOG.md
**Next Steps:** CRANE, ATLAS, and CODEX populate their own resume files on their next turn; all seats maintain theirs every turn going forward per RULE 21(3).
---

## 2026-09-03 10:45 - SCRIBE self-correction, RULE 22/23 added, W2-370 disk-verified and queued, conductor-error logged
**Action:** Self-correction first: an earlier attempt to disk-verify W2-370's M1-M4 milestones used `git merge-base --is-ancestor <branch-tip-sha> origin/main`, which came back NOT-LANDED for all four and led SCRIBE to refuse the annotation. The operator flagged this method as invalid — land.ps1 squashes and rewrites SHAs, so a branch-tip commit is never an ancestor of main even when the work fully landed. Re-verified using the correct method: `git ls-tree -r origin/main` confirmed apps/web/lib/analysis/** (7 modules + types + sampleData), apps/web/components/AnalysisTab.tsx (with M4's print-export addition), and apps/web/__tests__/analysis/** all present on origin/main; `git log origin/main --grep="[land:w2-370/..."` confirmed all four landing-marker commits (a56cbad3 M1, b4c03309 M2, 52cdd434 M3, c5dba000 M4) in main's own log. Both checks agreed — the work is real and landed. Added AGENTS.md RULE 22 (Self-contained prompts, no-stall queries) codifying this exact method plus its fallback (undecidable → log gate, continue non-dependent work, escalate the specific claim, never stall or fabricate), and RULE 23 (Every relay improves the system) as the conductor-side counterpart to RULE 17's seat-side UX-proposal requirement. Mirrored both into docs/seats/CRANE.md, ATLAS.md, CODEX.md, SCRIBE.md. Created W2-370 in docs/WAVE_QUEUE.md (it had never been queued despite the work landing) as LANDED-ON-MAIN (M1-M4) / DONE (M5), citing landing-marker SHAs as disk proof and original branch-tip SHAs as authorship provenance only. Logged the conductor-error entry with the exact facts the operator supplied: the conductor conflated RIVET's already-pushed icon patch (7c73ca66, +240/-240) with RIVET's separate uncommitted studio-docs work (+225/-0, 3 files) when relaying a task; CRANE's new session caught the scope mismatch and held; the conductor logged this as its own attribution error, prompt-level, no faulty repo artifact behind it.
**By:** SCRIBE (Claude Code)
**Status:** ✅ Complete
**Files Modified:** AGENTS.md, docs/seats/ATLAS.md, docs/seats/CODEX.md, docs/seats/CRANE.md, docs/seats/SCRIBE.md, docs/WAVE_QUEUE.md, docs/ACTIVITY_LOG.md
