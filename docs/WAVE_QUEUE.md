...
| W2-346 | | B2 | J08 | CODEX | OPEN | | SITEWIDE_CLAIM_TRUTH — copy side: execute every REWRITE/ROADMAP-LABEL/DELETE decision from W2-345's inventory that is a content/copy change (no code wiring) — correct or soften claim text, add roadmap labels, remove claims with no path to truth this phase. Reassigned ATLAS → CODEX (2026-09-02): falls in the CODEX slice (346, 348, 349, 350, 353, 354). UNDO: git revert <sha>. |
| W2-347 | | B2 | J08 | CRANE | OPEN | | SITEWIDE_CLAIM_TRUTH — tools side: execute every IMPLEMENT-MIN/WIRE decision from W2-345's inventory that requires connecting or minimally implementing a feature so the claim becomes true. Reassigned ATLAS → CRANE (2026-09-02): tools-side wiring is worker.ts/MCP territory, CRANE-only per the disjoint-ownership protocol; specific reassignment overrides the CODEX 346-350 roster range (2026-09-02 confirmed). Note: CommunityBuild investor-KYC wiring is Stage-2, BLOCKED per docs/COMPLIANCE_GATE.md — stays ROADMAP-LABEL, not IMPLEMENT-MIN/WIRE, until Stage-2 counsel sign-off; added as a Stage-2 candidate to docs/TRANSACTION_COUNSEL_PACK.md. UNDO: git revert <sha>. |
| W2-348 | | B2 | J08 | CODEX | OPEN | | CONTENT_ASSETS — templates/whitepapers/reports pages currently promise downloadable assets; make them real. Generate actual DXF/PDF/checklist files server-side or as static assets so every "download" affordance on those three resource hubs produces a real file, not a dead link or placeholder. Assigned CODEX per the CODEX slice (346, 348, 349, 350, 353, 354) — consistent with CRANE's own release-claim landed 2026-09-02. UNDO: git revert <sha>. |
| W2-349 | | B2 | J08 | CODEX | OPEN | | MEDIA_HONESTY — videos/podcasts pages: no recorded media exists and none can be faked. Convert each entry to article-format episode notes (real written content), OR roadmap-label it as not-yet-available, OR delete the watch/listen affordance entirely — no page may present a play/watch/listen control that doesn't play anything. Reassigned ATLAS → CODEX (2026-09-02): falls in the CODEX slice (346, 348, 349, 350, 353, 354). UNDO: git revert <sha>. |

## 2026-09-02 — SCRIBE dead-code sweep + copy-count sweep (RULE 13 extrapolation)
| W2-350 | | B2 | J08 | CODEX | OPEN | | DEAD_CODE_SWEEP — delete components/ProductPage.tsx + components/sections/Hero.tsx and every other never-imported component found by an import-graph sweep of the codebase. Build must stay green after removal. Reassigned ATLAS → CODEX (2026-09-02): falls in the CODEX slice (346, 348, 349, 350, 353, 354). UNDO: git revert <sha>. |
