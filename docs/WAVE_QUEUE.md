...
| W2-314 | | B2 | J08 | CRANE | OPEN | | author CONCIERGE_LLM grounding design doc — retrieval over the site catalog, mandatory citations on every generated answer, deterministic-router fallback when retrieval is unconfident, abuse/cost model with rate limits + budget caps. Research/docs only, no code. |
