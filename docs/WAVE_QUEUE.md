...
| W2-345 | | B2 | J08 | ATLAS | OPEN | | SITEWIDE_CLAIM_TRUTH — audit: per RULE 13 (screenshot extrapolation), inventory EVERY claim on EVERY page: feature cards, marketing bullets (e.g. "Escrow payments", "Verified pros", "KYC/AML verification"), hero claims, FAQ answers, pricing tier promises, how-it-works steps, resources descriptions. For each claim, record backing status (what's actually implemented/wired vs. asserted) and a decision: IMPLEMENT-MIN, WIRE, ROADMAP-LABEL, REWRITE, or DELETE. Deliverable: full claim inventory table + decisions. UNDO: git revert <sha>. |
| W2-346 | | B2 | J08 | MASON | OPEN | | SITEWIDE_CLAIM_TRUTH — copy side: execute every REWRITE/ROADMAP-LABEL/DELETE decision from W2-345's inventory that is a content/copy change (no code wiring) — correct or soften claim text, add roadmap labels, remove claims with no path to truth this phase. Reassigned ATLAS → MASON (2026-09-02): falls in the MASON slice (346, 348, 349, 350, 353, 354). UNDO: git revert <sha>. |
| W2-347 | | B2 | J08 | CRANE | OPEN | | SITEWIDE_CLAIM_TRUTH — tools side: execute every IMPLEMENT-MIN/WIRE decision from W2-345's inventory that requires connecting or minimally implementing a feature so the claim becomes true. Reassigned ATLAS → CRANE (2026-09-02): tools-side wiring is worker.ts/MCP territory, CRANE-only per the disjoint-ownership protocol; specific reassignment overrides the MASON 346-350 roster range (2026-09-02 confirmed). Note: CommunityBuild investor-KYC wiring is Stage-2, BLOCKED per docs/COMPLIANCE_GATE.md — stays ROADMAP-LABEL, not IMPLEMENT-MIN/WIRE, until Stage-2 counsel sign-off; added as a Stage-2 candidate to docs/TRANSACTION_COUNSEL_PACK.md. UNDO: git revert <sha>. |
...
## 2026-09-02 — SCRIBE dead-code sweep + copy-count sweep (RULE 13 extrapolation)
| W2-350 | | B2 | J08 | MASON | OPEN | | DEAD_CODE_SWEEP — delete components/ProductPage.tsx + components/sections/Hero.tsx and every other never-imported component found by an import-graph sweep of the codebase. Build must stay green after removal. Reassigned ATLAS → MASON (2026-09-02): falls in the MASON slice (346, 348, 349, 350, 353, 354). UNDO: git revert <sha>. |
