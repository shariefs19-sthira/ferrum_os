...
## 2026-09-03 — SCRIBE land.ps1 targeting tooling row
| W2-398 | | B2 | J08 | CRANE | OPEN | | LAND_TARGETING — land.ps1 gains a `-Branches` parameter accepting one or more explicit branch names/patterns, so a targeted land can be requested without either (a) manually squash-merging outside the script's own safety machinery (protected-path pre-flight refusal, hold-list respect, type-check-before-push) or (b) running the full catch-all sweep of every origin/w2-* branch when only specific branches are ready. When `-Branches` is supplied, only those branches are processed — the pre-flight refusal, hold-list check, and rebase-then-squash docs path all still apply per-branch exactly as in the catch-all mode. Per RULE 21(1), emits the same processed/landed/skipped/held counts, scoped to just the requested branches. Exemplar motivating this row (2026-09-03): SCRIBE was asked to land three specific docs branches via land.ps1 "now," but the script had no way to do that without either sweeping every pending branch (CRANE's gate, not SCRIBE's to trigger) or falling back to manual `git merge --squash` outside the script's safety net. UNDO: git revert <sha>. |
