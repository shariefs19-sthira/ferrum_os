...
| W2-354 | | B2 | J08 | MASON | OPEN | | RESPONSIVE_SWEEP — sequenced AFTER W2-353. Every route × viewports 375×667, 390×844, 768×1024, 1024×768, 1366×768, 1920×1080 + landscape phone. Checks: zero horizontal overflow; nav/hamburger functional; footer grid collapses cleanly; hero composed-visuals scale unclipped; cards stack; tables stack or scroll; tap targets ≥44px; every calculator/tool usable at 375px; type legible; viewport meta present (PWA-ready). Acceptance: Playwright routes × breakpoints matrix with zero violations post-fix; re-crawl green; build + verify-static green. Re-sequenced (2026-09-02): now also runs AFTER W2-366 COMMAND_DECK_UI, since /account (the Command Deck) must exist before it's included in the responsive matrix. Assignee corrected CODEX → MASON (2026-09-02, 12:51 AM claim) — stale name from the pre-rename branch chain (see W2-356). UNDO: git revert <sha>. |
...
## 2026-09-02 — SCRIBE Command Deck (backend + UI, sequenced with 362/354/363)
| W2-365 | | B2 | J08 | CRANE | OPEN | | COMMAND_DECK_BACKEND — migrations: projects table + project_id column on artifacts/leads/parcels; additive and idempotent, applied to local AND production under the same one-file/verify protocol. REST + MCP parity for project CRUD + attach operations. Per-user scoping enforced server-side (not just UI-hidden). Sequencing: runs after W2-362 (ALL_TOOLS_LIVE_SWEEP) lands. UNDO: git revert <sha> + drop projects table / project_id columns via a paired down-migration. |
| W2-366 | | B2 | J08 | MASON | OPEN | | COMMAND_DECK_UI — assigned MASON on the assumption its rate limit has reset; whoever claims it (conductor reassigns to CRANE instead if MASON is still limited) should update this row's Assigned To before starting. /account becomes the Command Deck per conductor spec: pipeline spine, engine launchers with resume, activity ledger, Ctrl+K command palette, honest state chips (no fake "live" states). Relume tokens, dense data-forward layout. Depends on W2-365 (backend) landing first. UNDO: git revert <sha>. |
...
| W2-381 | W2-380 | B2 | J08 | CRANE | OPEN | | S1 PARCEL_INTEL — ULPIN lookup → plot/zoning/FAR/society building types/advisable types. Versioned per-city SAMPLE DCR+FAR rulesets (INDICATIVE, schema ready for real data to replace samples later). Advisable-types rule engine. Sequencing: after CRANE's M5 milestone. UNDO: git revert <sha>. |
