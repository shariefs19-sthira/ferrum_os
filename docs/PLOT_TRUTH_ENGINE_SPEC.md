...
| **Landeed** (private aggregator) | Private, commercial | **Quantified 2026-09-08 (`docs/LANDEED_DEEP_DIVE.md` §1b, independently observed in-app, not company-claimed): Andhra Pradesh deepest at ~48 offerings (28-badge undercounts the real list); Telangana 6; Madhya Pradesh 3; 18 other states/UTs at exactly 2; 17 states/UTs — mostly North-East/hill regions — at 0.** The "23+ states" marketing figure is technically accurate as a list-membership count but materially overstates actual depth for most listed states | Derived from official state databases (per vendor claim); vendor's own product app states **"not affiliated with any government agency,"** in tension with the marketing site's separate "Partnership with Government" claim — reconciliation unclear | **Commercial API** — the only confirmed API-based multi-state access path found this pass | Paid (pricing not published/found this pass) | Private vendor's own ToS govern — **not verified this pass**; using it means depending on a third party's own compliance with state ToS, an added risk layer |
